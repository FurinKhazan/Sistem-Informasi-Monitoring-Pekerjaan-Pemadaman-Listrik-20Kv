   ...
   public function __construct()
	...
	public function index()
	{
      ...
      $data ['user'] = $this->User_model->getJoinUser1($iduser);
       ...
       $config['base_url']= base_url('/staff/permohonan/index');
  ...
  public function tambah()
   {
      ...
      $data ['user'] = $this->User_model->getJoinUser1($iduser);
       ...
       $this->form_validation->set_rules('tanggal_pekerjaan','Tanggal Pekerjaan','required|date');//SETING VALIDASI FORM TAMBAH| ('name','pesan yang akan di munculkan','metode vlidasi')
       ...
       if ($this->form_validation->run() == FALSE) //VALIDASI
                ...
                {
                  $this->Permohonan_model->tambahDatapermohonan();
                  $this->session->set_flashdata('flash','Ditambahkan');
                  redirect('staff/permohonan'); //REDIRECT KE CONTROLLER vendor
                }
    }
    ...
    public function ubah($id)
    {
      ...
      $data ['user'] = $this->User_model->getJoinUser1($iduser);
       ...
       if ($this->form_validation->run() == FALSE) //VALIDASI
                ...
                {
                  $this->Permohonan_model->ubahDatapermohonan();
                  $this->session->set_flashdata('flash','Diubah');
                  redirect('staff/permohonan'); //REDIRECT KE CONTROLLER PEGAWAI
                }
    }
    public function keputusan($id)
    {
       $data['judul'] = 'Keputusan Data permohonan';
       $data ['permohonan1'] = $this->Permohonan_model->getJoinPermohonan1($id);
       $data ['permohonan'] = $this->Permohonan_model->datapermohonanById($id);
       
       $data ['ulp']= $this->Ulp_model->getAllUlp1();
       $data ['penyulang']= $this->Penyulang_model->getAllPenyulang1();
       $data ['pegawai']= $this->Pegawai_model->getAllPegawai1();
       $data ['vendor']= $this->Vendor_model->getAllVendor1();
       $email= $this->session->userdata('email');
      $user = $this->db->get_where('user',['email' => $email])->row_array();
      ...
      //           }
       if ($this->form_validation->run() == FALSE) //VALIDASI
                ...
                else
                {
                  ...
                  $this->session->set_flashdata('flash','Diubah'); 
                  // $this->Permohonan_model->hapusDataPermohonan($id);
                  redirect('staff/perijinan'); //REDIRECT KE CONTROLLER izin
